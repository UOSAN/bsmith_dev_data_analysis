...
boot_times<-100
include_hispanic_status <- TRUE
if(include_hispanic_status){
  race_vars <- "RacePP+Hisp_Cat2"
}else{
  race_vars <- "RacePP"
}
...
formula_rhs<-paste0("~HINTSIsRural_i+Age_c+I(Age_c^2)+",race_vars,"+Gender3C+filename")
...
run_HINTS_mediation <- function(outcome,mediator,is_logistic=FALSE){
  ...
  mb_formula <<-paste0(mediator,"~HINTSIsRural_i+Age_c+I(Age_c^2)+",race_vars,"+Gender3C+filename")
  mb <<- lm(eval(mb_formula),data_to_measure)
  mc_formula<<- paste0(outcome, "~", mediator, "+HINTSIsRural_i+Age_c+I(Age_c^2)+",race_vars,"+Gender3C+filename")
...
mediation_result1 = run_HINTS_mediation("GeneralHealth_i", "Fruit_vegetables_z", is_logistic=FALSE)
```
...
mediation_result2 = run_HINTS_mediation("MedConditions_HighBP_i", "Fruit_vegetables_z", is_logistic=TRUE)
...
set.seed(9966743)
mediation_result3 = run_HINTS_mediation("EverHadCancer_i", "Fruit_vegetables_z", is_logistic=FALSE)
...
mediation_result4 = run_HINTS_mediation("GeneralHealth_i", "IncomeRanges_r", is_logistic=FALSE)
```
...
mediation_result5 = run_HINTS_mediation("MedConditions_HighBP_i", "IncomeRanges_r", is_logistic=TRUE)
...
set.seed(30738241)
mediation_result6 = run_HINTS_mediation("MedConditionsAggregate", "IncomeRanges_r", is_logistic=FALSE)
...
set.seed(76567222)
mediation_result7 = run_HINTS_mediation("EverHadCancer_i", "IncomeRanges_r", is_logistic=FALSE)

```

```{r}
save.image(file=paste0(config::get("rural_data_dir"),"HINTS_health_outcomes_bt_",as.character(boot_times),"_hs_",as.character(include_hispanic_status),".Rdata"))
# load(file=paste0(config::get("rural_data_dir"),"HINTS_health_outcomes_bt_",as.character(boot_times),"_hs_",as.character(include_hispanic_status),".Rdata"))
```
