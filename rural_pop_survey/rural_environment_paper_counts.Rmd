...
```{r}
orfcs_dataset_rz<-orfcs_dataset[!is.na(orfcs_dataset$RuralZIP),]
age_na<-is.na(orfcs_dataset_rz[,c("DEMO_Age_c")])
table(age_na)
orfcs_dataset_cumul<-orfcs_dataset_rz[!age_na,]
colSums(is.na(orfcs_dataset_cumul[,c("BMI","cancer_promoting_minus_preventing_FFQ","RuralZIP",
                      "PSS_z","Gender3C","RaceC2")]))
...
print(length(predict_ffq_rural$residuals))
...
print(length(predict_bmi_rural$residuals))
```
...
```{r}
print(length(mb$residuals))
print(length(mc$residuals))
```
