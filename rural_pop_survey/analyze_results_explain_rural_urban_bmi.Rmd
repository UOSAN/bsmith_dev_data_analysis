...
```
```{r}
preprocessed_data$FFQ_fruits_ln_z <- as.numeric(preprocessed_data$FFQ_fruits) %>% log %>% (function(x){(x-mean(x,na.rm=TRUE))/sd(x,na.rm=TRUE)})
preprocessed_data$FFQ_vegetables_ln_z <- as.numeric(preprocessed_data$FFQ_vegetables) %>% log %>% (function(x){(x-mean(x,na.rm=TRUE))/sd(x,na.rm=TRUE)})
```
