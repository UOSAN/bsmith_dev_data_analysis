...
hints5 <-get_hints5(location="/Users/Ben/Dropbox (University of Oregon)/protected_files_ben_only")
boot_times<-10
...
# Summarizing the HITNS dataset results

Because there are quite a lot of results here, we should probably summarize them in a table.

```{r}
lavaan_results_df %>% filter(label %in% c(
  "indirect_effect",
  "direct_effect",
  "effect_via_inc_only",
  "effect_via_fruit_vege_only")) %>%
  select(label, outcome,est,se,pvalue) %>% arrange(label,outcome) %>% gt::gt() %>% gt::as_raw_html()
