...
ppt_cols <-c("aid","HEIGHT_1","HEIGHT_2","WEIGHT","RuralZIP","RUCA_Description")
...
scored_wide <- scored %>% select(SID,scale_subscale_name,score) %>% 
...
ppts_by_row <- merge(scored_wide,non_scale_data,by.x="SID",by.y="aid",all=TRUE) %>% 
  merge(ppts_via_csv,by.x="SID",by.y="aid",all=TRUE) %>% 
  merge(promote_minus_prevent,by="SID",all=TRUE)
