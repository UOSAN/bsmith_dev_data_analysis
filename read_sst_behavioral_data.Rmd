...
```{r}
get_formatted_seeker_from_mat_file <-function(fname,raw_behavioral_data_filepath){
  finfo <- str_match(fname,"(DEV\\d{3})_run(\\d)_(\\d*-[a-zA-Z]*-\\d*)\\_(\\d*-\\d*.*)\\.mat")
    ...
    colnames(seeker) <- c("trial_n","numchunks_reciprocal","go_no_go_condition","arrow_presented","ladder_number","LadderX_SSD_ms","subject_response","ladder_movement","reaction_time","SSD_technical","SSD_observed","onset","arrow_appearance_t","SSD_recorded",
                          "trial_duration","time_course","health_condition")
    seeker$subid<-subid
    ...
    seeker <- seeker %>% mutate(
      health_condition_label = case_when(
            health_condition == 0 ~ "Unhealthy",
            health_condition == 1 ~ "Healthy",
            health_condition == 2 ~ as.character(NA),
            TRUE ~ as.character(NA)
          )
      )
...
#subj<-subj_ids[[1]]
  ...
  for(fname in subj_fnames){
    #fname<-fname_list[[1]]
    seeker <- get_formatted_seeker_from_mat_file(fname,raw_behavioral_data_filepath)
...
```
Let's add some additional important information.

This will be the jitter and the "ODI" which is the delay between trial onset and arrow onset

```{r}


jitter_values = readr::read_table(paste0(sst_on_dropbox,'/output/jitter.txt'),skip=1,col_names="jitter")

jitter_values_df = data.frame(
  'trial_n'= (1:nrow(jitter_values))*2-1,
  'jitter_value'=jitter_values$jitter)
study_df_aug = merge(study_df,jitter_values_df,by = 'trial_n', how='left',all.x = TRUE,sort = FALSE) %>%
  arrange(subid, waveid, runid, trial_n)
study_df_aug$OCI = 0.5 + study_df_aug$jitter_value
study_df_aug$onset_arrow <- study_df_aug$OCI + study_df_aug$onset 
```


```{r}
readr::write_csv(study_df_aug,file = paste0(data_path,"sst_behavioral_data_all.csv"))

```

#separate source of SSRT

```{r}
source("SST_processing.R")

sst_all_data_raw <- study_df_aug
sst_all_data<-clean_sst_data(sst_all_data_raw)
sst_all_data <- get_expected_tone_time(sst_all_data)
sst_all_data <- label_button_presses(sst_all_data)
...
sst_all_data <- get_more_sst_trial_stats_multiple_versions(sst_all_data)
...
readr::write_csv(sst_all_data,file = paste0(data_path,"sst_behavioral_data_all_comprehensive.csv"))
...
I'm going to check the data above vs. Redcap-derived data to identify any data that might be missing which we should have.
...
condition_count <- sst_all_data %>% group_by(runid,subid,condition) %>% count() %>% ungroup()
trial_summary_data <- sst_all_data %>% group_by(runid,waveid, subid) %>% 
  summarise(go_trial_reaction_time = mean(reaction_time[condition %in% c("CorrectGo")]),
            #reaction_time = mean(reaction_time[condition %in% c("CorrectStop")],na.rm=TRUE),
            mean_ssrt_0 = mean(ssrt_0[condition=="CorrectStop"],na.rm=TRUE),
            mean_ssrt_0_congruent_only = mean(ssrt_0_congruent_only[condition=="CorrectStop"],na.rm=TRUE),
            mean_SSD = mean(SSD_recorded[condition %in% c("CorrectStop","FailedStop")],na.rm=TRUE)
            ) %>% ungroup()
...
summary_stats_by_run <- prop_successful_stops %>% merge(trial_summary_data)
...
#old_format <- readr::read_csv(file=paste0(data_path,"sst_summary_stats_by_run.csv"))
```
...
# Test combining
```{r}
combined_subject_stats <- merge(summary_stats_by_run,presummarized_data_all,by.x=c("subid","waveid"),by.y=c("SubjectLabel","wave"))
colnames(combined_subject_stats)
cor(combined_subject_stats %>% select("SSRT","SSRTint","SSRTquant","mean_ssrt_0","mean_ssrt_0_congruent_only"),use = "pairwise.complete.obs")
```
OK, these are very close; what's the remaining difference between SSRTquant and mean_ssrt_0 attributable to?

Considering the subject pattern, perhaps it is labels of left and right? Shouldn't be, because those aren't even l

```{r}
plot(combined_subject_stats$mean_ssrt_0,combined_subject_stats$SSRTquant)
plot(combined_subject_stats$mean_ssrt_0_congruent_only,combined_subject_stats$SSRTquant)
```
