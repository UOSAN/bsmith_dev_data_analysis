...
model1 <- lme4::lmer(
  rating_norm ~ run + stimulus_f + hunger_norm + (1+ stimulus_f | subjectID),
  regress_data
  )
summary(model1)
...
#commented out because we don't have the specific food data currently
# 
# model2 <- lme4::lmer(
#   rating_norm ~ run + health_rating + stimulus_f*hunger_norm + (1 + stimulus_f | food_pic) + (1+ stimulus_f + health_rating | subjectID),
#   regress_data
#   )
# 
# summary(model2)
...
# 
# model3 <- lme4::lmer(
#   rating_norm ~ run + health_rating*hunger_norm + stimulus_f*hunger_norm + (1+ stimulus_f + health_rating | subjectID),
#   regress_data
#   )
# 
# summary(model3)
...
)+geom_bar(stat="identity")+facet_wrap(~hunger_1)+
...
grouped_regress_data <- regress_data %>% group_by(subjectID) %>% summarize(
...
model_no_liking <- lme4::lmer(
  rating_norm ~ run + health_rating + (1 + health_rating | subjectID),
  regress_data
  )
...
model_no_health <- lme4::lmer(
  rating_norm ~ run + stimulus_f + (1+ stimulus_f | subjectID),
  regress_data
  )
...
model_liking_health <- lme4::lmer(
  rating_norm ~ run + stimulus_f +health_rating + (1+ stimulus_f +health_rating | subjectID),
  regress_data
  )
