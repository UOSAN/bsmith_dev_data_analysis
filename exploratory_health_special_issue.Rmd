...
ppt_health_outcomes <- ppt_list_w_data %>% select(dev_id,bf_1,weight_0) %>% filter(!is.na(bf_1))
rm(ppt_list_w_data)
...
sst_data <- readr::read_csv(file=paste0(data_path,"sst_summary_stats_by_run.csv"))
...
sst_w1 <- sst_data%>%filter(runid==1)%>% select(-runid)
...
Now we get the behaivoral data
...
IVs <- c("cancer_promoting_minus_preventing_craved_FCI","cancer_promoting_minus_preventing_liked_FCI",
         ...
         "EDM","BSCS","ACES_sum",
         "SST_prop_successful_stops","ROC_Crave_Regulate_Minus_Look","ROC_Crave_Regulate",
