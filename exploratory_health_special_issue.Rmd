...
print(summary(lm(bf_1~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt)))



print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density+FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt)))

```
```{r}
print(summary(lm(bf_1~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt %>% filter(birthsex==1))))
print(summary(lm(bf_1~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt %>% filter(birthsex==2))))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt %>% filter(birthsex==1))))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt %>% filter(birthsex==2))))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt %>% filter(birthsex==1))))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt %>% filter(birthsex==2))))
```


```{r}

#try out a difference score.
data_by_ppt$FFQ_v2_Mean_Nutrient_Minus_Energy_Density <- data_by_ppt$FFQ_v2_Mean_Weighted_Nutrient_Density - data_by_ppt$FFQ_v2_Mean_Energy_Density
```



```{r fig.width=12}
corrplot::corrplot(cor(data_by_ppt[,c("bf_1","bmi","cancer_promoting_FFQ","cancer_preventing_FFQ","cancer_promoting_minus_preventing_FFQ","FFQ_v2_Mean_Energy_Density",
"FFQ_v2_Mean_Nutrient_Minus_Energy_Density","FFQ_v2_Mean_Weighted_Nutrient_Density","FFQ_v2_Mean_Weighted_percent_daily_value","FFQ_v2_Mean_Dietary_Nutrient_Density")],use = "pairwise.complete.obs"),
method="number",type="upper",diag=FALSE,tl.srt=35,tl.cex=0.8,cl.cex=0.8)
```
...
```{r, fig.width=14}
corrplot::corrplot(cor(data_by_ppt[,c("bf_1","bmi","cancer_promoting_minus_preventing_FFQ","FFQ_v2_Mean_Energy","FFQ_v2_Mean_Energy_Density", "FFQ_v2_Mean_Weighted_Nutrient_Density","FFQ_v2_Mean_Weighted_percent_daily_value",
                                      "FFQ_v2_Mean_Dietary_Nutrient_Density","FFQ_v2_Mean_Nutrient_Minus_Energy_Density",
      "WTP_unhealthy_minus_healthy", #valuation
      "cancer_promoting_minus_preventing_craved_FCI", "ROC_Crave_Minus_Neutral",#craving
      "SRHI_healthy_minus_unhealthy",#habit
      "TESQ_E_goal_and_rule_setting", "TESQ_E_sum", #self control
      "ses_aggregate"                                #SES
                                      )],use = "pairwise.complete.obs"),
method="number",type="upper",diag=FALSE,tl.srt=40,tl.cex=0.7,cl.cex=0.8)
```

```{r}
print(summary(lm(bf_1~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Energy,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Energy_Density,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Dietary_Nutrient_Density,data_by_ppt)))
print(summary(lm(bf_1~FFQ_v2_Mean_Weighted_Nutrient_Density+FFQ_v2_Mean_Energy_Density,data_by_ppt)))
```


```{r}

print(summary(lm(bmi~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt)))
print(summary(lm(bmi~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt %>% filter(birthsex==1))))

print(summary(lm(bmi~cancer_preventing_FFQ+cancer_promoting_FFQ,data_by_ppt %>% filter(birthsex==2))))

print(summary(lm(bmi~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt %>% filter(birthsex==1))))
print(summary(lm(bmi~FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt %>% filter(birthsex==1))))
print(summary(lm(bmi~FFQ_v2_Mean_Weighted_Nutrient_Density,data_by_ppt %>% filter(birthsex==2))))
print(summary(lm(bmi~FFQ_v2_Mean_Weighted_percent_daily_value,data_by_ppt %>% filter(birthsex==2))))
```
...
- All FFQ scales perform about equally well correlating with BF%, around 0.18 to 0.22, except dietary nutrient density and Cancer Promoting Preventing Difference (the original FFQ), which correlate less well.
 - Energy Density correlates most with BMI, followed by Promoting Preventing Difference, and others don't relate to BMI much at all. Personally I think positively correlating with BF% is important, but a relationship with BMI is probably not at all important in the DEV context.
 - For the two behavioral measures examined, based on WTP and ROC, Dietary Nutrient Density is the strongest correlate of all, followed by Cancer Promoting Minus Preventing, followed by Nutrient Density measures. Energy density measures don't perform well at all.
 - For the self-report measures (SRHI, TESQ), Dietary Nutrient Density performs best of all, followed by the nutrient density measures. Energy density doesn't do well at all.
 - For the SES measure, nurtrient density measures perform best, followed by cancer promoting preventing difference. 
...
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_sum,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_sum*birthsex_factor,data_by_ppt))

summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_controlling_temptations*birthsex_factor,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_distraction*birthsex_factor,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_goal_and_rule_setting*birthsex_factor,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_goal_deliberation*birthsex_factor,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_suppression*birthsex_factor,data_by_ppt))
summary(lm(cancer_promoting_minus_preventing_FFQ~TESQ_E_suppression,data_by_ppt%>% filter(birthsex_factor=="Male")))
```
...
```{r}
cor.test(data_by_ppt$SST_PostErrorSlowW1_median,data_by_ppt$RTFS_f1_minus_f2)
cor.test(data_by_ppt$SST_PostErrorSlowW1_median,data_by_ppt$RTFS_factor_1)
cor.test(data_by_ppt$SST_PostErrorSlowW1_median,data_by_ppt$RTFS_factor_2)
cor.test(data_by_ppt$SST_PostErrorSlowW1_mean,data_by_ppt$RTFS_factor_2)
cor.test(data_by_ppt$SST_pes_mean_limited,data_by_ppt$RTFS_f1_minus_f2)
cor.test(data_by_ppt$SST_pes_mean_limited,data_by_ppt$RTFS_factor_1)
cor.test(data_by_ppt$SST_pes_mean_limited,data_by_ppt$RTFS_factor_2)


sst_pes_simple_old <- readr::read_csv(paste0("~/Dropbox (University of Oregon)/UO-SAN Lab/Berkman Lab/Devaluation/analysis_files/data/", "post_error_slowing_simple_approach_OLD.csv"))

merge(data_by_ppt %>% select(SID, SST_PostErrorSlowW1_median,SST_PostErrorSlowW1_mean), 
      sst_pes_simple_old %>% select(subid, PostErrorSlowW1)
      ,by.x="SID",by.y="subid")
```
...
```{r}
summary(lm(cancer_promoting_minus_preventing_FFQ~RTFS_factor_1*birthsex, data_by_ppt))
```
...
print(summary(lm(bf_1~PTO_RS+IPAQ_combined,data_by_ppt)))
...
```{r}
load("~/Dropbox (University of Oregon)/UO-SAN Lab/Berkman Lab/Devaluation/analysis_files/data/raw_survey_data.RData")
surveys_long_clean %>% filter(grepl("FFQ_",item)) %>% select(SID,item,value)

# ffq_detailed <- surveys_long_clean %>% 
#   filter(grepl("FFQ_",item) & survey_name=="DEV Session 1 Surveys") %>% 
#   select(SID,item,value) %>% pivot_wider(id_cols="SID",names_from="item",values_from="value")

ffq_scoring_rubric <- readr::read_csv("/Users/benjaminsmith/Dropbox (University of Oregon)/UO-SAN Lab/Berkman Lab/Devaluation/DEV_scoring_rubrics/FFQ_scoring_rubric.csv")

cancer_promoting_items <- ffq_scoring_rubric %>% filter(`Cancer Promoting`==1) %>% .$`Column Name`


ffq_cancer_promoting_detailed <- surveys_long_clean %>% 
  filter(grepl("FFQ_",item) & survey_name=="DEV Session 1 Surveys") %>% 
  select(SID,item,value) %>% filter(item %in% cancer_promoting_items)
  

ffq_cancer_promoting_detailed$value <- as.integer(ffq_cancer_promoting_detailed$value)
...
```{r fig.width=12}
ggplot(ffq_cancer_promoting_detailed,aes(value))+geom_histogram()+
...
```{r}
hist(log(data_by_ppt$cancer_promoting_FFQ))

data_by_ppt$cancer_promoting_FFQ_custom<- data_by_ppt$cancer_promoting_FFQ-1
print(summary(lm(bf_1~cancer_promoting_FFQ + IPAQ_total_METminutes, data_by_ppt)))
print(summary(lm(bf_1~cancer_promoting_FFQ_custom + IPAQ_total_METminutes, data_by_ppt)))
data_by_ppt$cancer_promoting_FFQ_custom<- log(data_by_ppt$cancer_promoting_FFQ)

data_by_ppt$cancer_promoting_FFQ_custom<- data_by_ppt$cancer_promoting_FFQ^2
print(summary(lm(bf_1~cancer_promoting_FFQ_custom + IPAQ_total_METminutes, data_by_ppt)))
```
...
library(dplyr)
library(tidyr)
ffq_detailed_wide <- ffq_cancer_promoting_detailed %>% pivot_wider(id_cols="SID",names_from="item",values_from="value")
ffq_detailed_w_extra<-merge(ffq_detailed_wide,data_by_ppt %>% select(SID,bf_1,IPAQ_total_METminutes,cancer_promoting_FFQ))
...
ffq_detailed_w_extra <- ffq_detailed_w_extra %>% rename(IPAQ = IPAQ_total_METminutes) %>% rename(cpFFQ = cancer_promoting_FFQ)
corrplot(cor(ffq_detailed_w_extra %>% select(-SID),use = "pairwise.complete.obs"),method="number")

summary(lm(as.formula(paste0("bf_1 ~ ",paste(colnames(ffq_detailed_w_extra)[grepl("FFQ_",colnames(ffq_detailed_w_extra))],collapse = " + "))),ffq_detailed_w_extra))

```

look at all items and look at the suqres...

```{r}
ffq_detailed <- surveys_long_clean %>% 
  filter(grepl("FFQ_",item) & survey_name=="DEV Session 1 Surveys") %>% 
  select(SID,item,value)
ffq_detailed$value<-as.integer(ffq_detailed$value)

#get raw data
ffq_detailed_wide <- ffq_detailed %>% pivot_wider(id_cols="SID",names_from="item",values_from="value")

#ffq_detailed_w_extra<-merge(ffq_detailed_wide,data_by_ppt %>% select(SID,bf_1,IPAQ_total_METminutes,cancer_promoting_FFQ))

# square it
ffq_items_only <- ffq_detailed_wide %>% select(-SID)
ffq_detailed_squared_all_items <- cbind(SID=ffq_detailed_wide$SID, ffq_items_only^2)

#get the summarized vals
cancer_promoting_items <- ffq_scoring_rubric %>% filter(`Cancer Promoting`==1) %>% .$`Column Name`
cancer_preventing_items <- ffq_scoring_rubric %>% filter(`Cancer Preventing`==1) %>% .$`Column Name`
cancer_promoting_sq<-rowMeans(ffq_detailed_squared_all_items[cancer_promoting_items])
cancer_preventing_sq<-rowMeans(ffq_detailed_squared_all_items[,cancer_preventing_items])

sq_data_by_sid <-data.frame(SID=ffq_detailed_squared_all_items$SID,
      FFQ_cancer_promoting_sq=cancer_promoting_sq,
      FFQ_cancer_preventing_sq=cancer_preventing_sq)

#merge back in.
data_by_ppt<-merge(data_by_ppt,sq_data_by_sid,by="SID")
```


```{r}
summary(lm(bf_1~FFQ_cancer_preventing_sq+IPAQ_total_METminutes,data_by_ppt))
summary(lm(bf_1~FFQ_cancer_promoting_sq+IPAQ_total_METminutes,data_by_ppt))
summary(lm(bf_1~FFQ_cancer_preventing_sq+FFQ_cancer_promoting_sq+IPAQ_total_METminutes,data_by_ppt))

#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4739486/
library(gtools)

data_by_ppt$IPAQ_quartiles <- quantcut(data_by_ppt$IPAQ_total_METminutes/60,4)
data_by_ppt$IPAQ_quartiles_int <- as.integer(data_by_ppt$IPAQ_quartiles)


summary(lm(bf_1~FFQ_cancer_preventing_sq+FFQ_cancer_promoting_sq+IPAQ_quartiles,data_by_ppt))
summary(lm(bf_1~FFQ_cancer_preventing_sq+FFQ_cancer_promoting_sq+IPAQ_quartiles_int,data_by_ppt))
summary(lm(bf_1~FFQ_cancer_promoting_sq+IPAQ_quartiles_int,data_by_ppt))
summary(lm(bf_1~FFQ_cancer_promoting_sq+IPAQ_MET_kCal+a,data_by_ppt))
