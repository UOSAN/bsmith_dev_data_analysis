...
sst_all_data_filepath_current <- paste0(config::get("dev_analysis_data_dir"),"sst_behavioral_data_all.csv")
sst_all_data_filepath_march <- paste0(config::get("dev_analysis_data_dir"),"sst_behavioral_data_all.3.7.22.csv")
...
sst_all_data_raw <- read.csv(file = sst_all_data_filepath, stringsAsFactors = FALSE, header = TRUE)
...
write.csv(sst_all_data_raw,paste0(config::get("dev_analysis_data_dir"),"SST_Raw2.csv"), row.names = FALSE)
...
write.csv(sst_PECGonly,paste0(config::get("dev_analysis_data_dir"),"SST_PEC.csv"), row.names = FALSE)
...
library(ggplot2)
psych::describe(sst_PECGonly$duration)
ggplot(data = sst_PECGonly) +
  geom_histogram(aes(x = duration), bins = 50, binwidth=.01 , boundary=-0.1, fill = 'turquoise', color = 'black', ) + theme(
...
sst_PECGonly <- subset(sst_PECGonly, duration>.25) 

#Remove all rows where duration is greater than 1.25 MS. 
sst_PECGonly <- subset(sst_PECGonly, duration<1.25) 

#Check that this occurred 
psych::describe(sst_PECGonly$duration) #looks good 

#Calculate the mean by participant for all Post-Error Correct Gos 
sst_PECG_byPp <- sst_PECGonly %>% group_by(subid) %>% summarise(PECG_mean = mean(duration, na.rm=TRUE))
...
library(ggplot2)
psych::describe(sst_PCCGonly$duration)
...
#Remove all rows where duration is less than 25 MS. 
sst_PCCGonly <- subset(sst_PCCGonly, duration>.25)

#Remove all rows where duration is greater than 1.25 MS. 
sst_PCCGonly <- subset(sst_PCCGonly, duration<1.25) 

#Check that this occurred 
psych::describe(sst_PCCGonly$duration) #looks good
...
sst_PCCG_byPp <- sst_PCCGonly %>% group_by(subid) %>% summarise(PCCG_mean = mean(duration, na.rm=TRUE))
...
sst_PECG_byPp_med <- sst_PECGonly %>% group_by(subid) %>% summarise(PECG_median = median(duration, na.rm=TRUE))

#Calculate the MEDIAN by participant for all Post-Error Correct Gos 
sst_PCCG_byPp_med <- sst_PCCGonly %>% group_by(subid) %>% summarise(PCCG_median = median(duration, na.rm=TRUE))
...
```{r}
write.csv(sst_PES,paste0(config::get("dev_analysis_data_dir"),"post_error_slowing_simple_approach.csv"))
